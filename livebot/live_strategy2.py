...
class Scalping:
    def __init__(self, symbol, data, threshold=0.001):
        self.symbol = symbol
    ...
    def generate_signals(self):
        self.data['price_diff'] = self.data[self.symbol].pct_change()
        self.data['buy_signal'] = np.where((self.data['price_diff'] > self.threshold), 1, 0)
        self.data['sell_signal'] = np.where((self.data['price_diff'] < -self.threshold), -1, 0)
    ...
    def extract_data(self):
        ...
        y_price_diff = self.data['price_diff']
